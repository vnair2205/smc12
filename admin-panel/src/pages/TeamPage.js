...
const TabContainer = styled.div` display: flex; border-bottom: 2px solid #2a2a3e; margin-bottom: 2rem; `;
...
const TeamPage = () => {
  ...
  return (
    <PageContainer>
      <Header><h1>Team Management</h1></Header>
      <TabContainer>
        <TabButton isActive={activeTab === 'department'} onClick={() => setActiveTab('department')}>Department</TabButton>
        <TabButton isActive={activeTab === 'designation'} onClick={() => setActiveTab('designation')}>Designation</TabButton>
        <TabButton isActive={activeTab === 'members'} onClick={() => setActiveTab('members')}>Team Members</TabButton>
      </TabContainer>
      <div>
        {activeTab === 'department' && <DepartmentManager />}
        {activeTab === 'designation' && <DesignationManager />}
        {activeTab === 'members' && <div>Team Members section coming soon.</div>}
      </div>
    </PageContainer>
  );
};
